...
public class TransportUDP : MonoBehaviour {
	...
	public string comm_address = "192.168.50.124";
	public int comm_port = 12345;
	...
	private byte[] mainBuffer = new byte[s_mtu];
	private int recvInd;
	...
	void Start ()
    {
		...
		//Connect(comm_address, comm_port);
		StartServer(comm_port,0);
	...
	void Update ()
	{
		recvInd = m_recvQueue.Dequeue(ref mainBuffer, mainBuffer.Length);
		if (recvInd > 0)
		{
			Debug.Log(mainBuffer);
		}
    ...
    public void Dispatch()
	{
		...
		while (m_threadLoop) {
			...
			if (m_socket != null && m_isConnected == true) {
				...
				Debug.Log("client is connected & socket is open");
	            // 송신처리.
	            DispatchSend();
